...
def build_agent_prompt(
    *,
    ...
    selected = [
        ...
        for item in context_core.get("selected_context", [])[:5]
    ]
    blocked = [
        {
            "title": item.get("title"),
        ...
        for item in context_core.get("blocked_context", [])[:3]
    ]
    compact_history = history[-8:]
    tool_state = {
        "tool_calls": [
            {
                "name": "fuze-context-core.query",
                "result": {
                    "retrieval": context_core.get("runtime", {}).get("retrieval"),
                    "embedding_source": context_core.get("runtime", {}).get("embedding_source"),
                    "graph_path": context_core.get("graph_traversal", {}).get("path", []),
                    "selected_context": selected,
                    "blocked_context": blocked,
                    "citations": context_core.get("citations", [])[:8],
                },
            },
            {
                "name": "workflow.tasks.list",
                "result": result.get("tasks", [])[:5],
            },
            {
                "name": "governance.approvals.list",
                "result": result.get("approvals", [])[:5],
            },
            {
                "name": "runtime.local_llm.proof",
                "result": result.get("audit", {}).get("model_runtime", {}).get("local_inference", {}),
            },
        ]
    }
    return (
        "you are fuze, a local-first enterprise agent running on a dell gb10. "
        "answer like a normal conversational product assistant, not a report template. "
        "be direct, lowercase, and concise. use only the tool results below for org facts. "
        "if the user asks whether llms are used, explain that the chat synthesis is generated by local ollama when enabled, "
        "while policy/retrieval/workflow tools keep the answer grounded. "
        "if the user asks what is happening, explain the current agent loop. "
        "never claim cloud llm calls; cloud llm calls are 0.\n\n"
        f"user message: {message}\n"
        f"standalone query after follow-up rewrite: {effective_goal}\n"
        f"recent chat history: {json.dumps(compact_history, ensure_ascii=False)}\n"
        f"tool results: {json.dumps(tool_state, ensure_ascii=False)}\n\n"
        "write the final chat answer now. include sources only when the user asks for org facts or when the answer depends on retrieved context."
    )
...
async def chat(request: ChatRequest) -> dict[str, Any]:
    ...
    context_core = await retrieval.query_context_core(
        question=effective_goal,
        role=request.role,
    ...
    llm_generation = await runtime.local_chat_completion(
        build_agent_prompt(
    ...
    if llm_generation.get("available") and llm_generation.get("response"):
        result["response"] = llm_generation["response"]
    ...
    return {
        **result,
        ...
        "context_core": {
            "server": context_core["server"],
            "runtime": context_core["runtime"],
            "selected_context": context_core["selected_context"],
            "blocked_context": context_core["blocked_context"][:3],
            "graph_path": context_core["graph_traversal"]["path"],
        },
        "llm_generation": llm_generation,
        "chat_runtime": {
            "mode": "tool-using local llm chat turn" if llm_generation.get("available") else "history-aware deterministic chat turn",
            "backend": "/chat",
            "tool_calls": [
                "fuze-context-core.query",
                "workflow.tasks.list",
                "governance.approvals.list",
                "runtime.local_llm.proof",
            ],
            "cloud_llm_calls": 0,
        },
    }
